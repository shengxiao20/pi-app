...
async function initializeAgent(
  ...
  const [directory, handoffSession] = await Promise.all([
  ...
  const history = await getHistory(client, metadata.path);
  const persistedSessions = await client.listSessions();
  const sessions = persistedSessions.map(workspaceSession);
  const matchingSession = sessions.find(
    ({ sessionPath }) => sessionPath === metadata.path,
  );
  if (!matchingSession)
    throw new Error("Pi did not list the active persisted session");
  const activeSession = {
    ...matchingSession,
