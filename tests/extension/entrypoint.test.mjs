...
test("the /app extension always launches desktop without a session handoff", () => {
  const source = readFileSync(appExtensionPath, "utf8");

  assert.doesNotMatch(source, /getSessionFile|existsSync|ctx\.shutdown/);
  assert.match(source, /launchDesktopClient\(\{ cwd: ctx\.cwd \}\)/);
});
